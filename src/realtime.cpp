...
//     SceneParser parser;
//     parser.parse("scenefiles/default.json", m_renderData);
// }
...
//         glUniformMatrix4fv(
//             glGetUniformLocation(m_gbufferShader, "model"),
//             1, GL_FALSE, &shape.ctm[0][0]);

//         glUniform3fv(
//             glGetUniformLocation(m_gbufferShader, "albedo"),
//             1, &shape.primitive.material.cDiffuse[0]);

//         glUniform3fv(
//             glGetUniformLocation(m_gbufferShader, "emissive"),
//             1, &shape.primitive.material.cEmissive[0]);

//         // ✅ FIX: cast PrimitiveType → int
//         int prim = static_cast<int>(shape.primitive.type);

//         glBindVertexArray(m_shapeVAOs[prim]);
//         glDrawArrays(GL_TRIANGLES, 0,
//                      m_shapeVertexCounts[prim]);
//     }

//     // ---- DYNAMIC SNAKE GEOMETRY (EMISSIVE) ----
//     std::vector<RenderShapeData> snakeShapes;
//     m_snake.submitGeometry(snakeShapes);

//     for (auto& shape : snakeShapes) {
//         glUniformMatrix4fv(
//             glGetUniformLocation(m_gbufferShader, "model"),
//             1, GL_FALSE, &shape.ctm[0][0]);
...
//     // Merge static + snake lights
//     std::vector<SceneLightData> allLights = m_renderData.lights;
//     m_snake.submitLights(allLights);

//     int numLights = std::min((int)allLights.size(), 8);
//     glUniform1i(
//         glGetUniformLocation(m_deferredShader, "numLights"),
//         numLights);
...
//         glUniform4fv(
//             glGetUniformLocation(m_deferredShader,
//                                  (base + ".pos").c_str()),
//             1, &L.pos[0]);
